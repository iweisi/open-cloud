...
在 src/test/java 下面，`generator` 包下面有一个测试类 `Generator`。
...
<generatorConfiguration>
    <context id="Mysql" targetRuntime="MyBatis3Simple" defaultModelType="flat">
        ...
        <!--通用代码生成器插件-->
        <plugin type="generator.TemplateFilePlugin">
...
```
在这个配置中，我们只关注 `generator.TemplateFilePlugin`。
...
<plugin type="generator.TemplateFilePlugin">
    ...
    <!--默认值是下面这个，可以不配置-->
    <property name="templateFormatter" value="generator.formatter.FreemarkerTemplateFormatter"/>
</plugin>
```
...
默认的实现类为：`generator.formatter.FreemarkerTemplateFormatter`。
...
<plugin type="generator.TemplateFilePlugin">
    ...
    <property name="templateFormatter"
              value="generator.formatter.FreemarkerTemplateFormatter"/>
</plugin>
...
还提供了一种 Java 编码方式运行的类，`src/test/java/` 中的 `generator.Generator`，配置上面 xml 中的数据库信息就可以生成。
...
所有配置的属性信息:
targetPackage - 
templateFormatter - generator.formatter.FreemarkerTemplateFormatter
